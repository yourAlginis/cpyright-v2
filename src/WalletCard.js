...
// let web3 = new Web3.providers.Web3Provider(window.ethereum);
//Contract Address
...
const WalletCard = () => {
   ...
   // important to update account if we have, will cause component re-render
	...
	const deploySetData = async ()=>{ 
     ...
     await contracts.setData(getVal).then(async(tx)=>{
		console.log(tx); 
		let acutualTx = tx.hash;
		setTxLink("https://rinkeby.etherscan.io/tx/".concat(acutualTx))
 
	    // let blockNumber= await web3.eth.getTransaction(tx).blockNumber
